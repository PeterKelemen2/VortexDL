...
<template>
  ...
  <Transition name="slide">
    <aside
      v-if="isOpen"
      class="fixed left-0 top-0 z-50 flex h-full w-72 flex-col bg-linear-to-b from-slate-900 to-slate-950 shadow-2xl"
    >
      <!-- Header -->
      <div class="flex items-center justify-between px-5 py-4 border-b border-white/10">
        <div class="flex items-center gap-2.5">
          ...
          <span class="text-base font-bold tracking-tight text-white">{{ BRAND_NAME }}</span>
        </div>
        <button
          type="button"
          @click="emit('close')"
      ...
      <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        <button
          ...
          :class="[
            ...
            isActive(item)
              ? 'bg-blue-500/20 text-blue-300 shadow-sm shadow-blue-900/30'
        ...
        >
          <component
            ...
            :class="[
              ...
              isActive(item) ? 'text-blue-400' : 'text-slate-500 group-hover:text-slate-300',
            ]"
          />
          {{ item.label }}
          <span v-if="isActive(item)" class="ml-auto h-1.5 w-1.5 rounded-full bg-blue-400" />
        </button>
      </nav>
      ...
      <div class="border-t border-white/10 px-3 py-3 space-y-1">
        ...
        <button
          type="button"
          @click="toggleTheme"
          class="group w-full flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-400 transition-all hover:bg-white/8 hover:text-white"
        >
          <component
            :is="isDark ? Sun : Moon"
            class="h-4 w-4 shrink-0 text-slate-500 group-hover:text-slate-300 transition-colors"
          />
          {{ isDark ? 'Light mode' : 'Dark mode' }}
          <!-- Toggle pill -->
          <span
            class="ml-auto flex h-5 w-9 shrink-0 items-center rounded-full bg-slate-700 px-0.5 transition-colors"
            :class="isDark ? 'bg-blue-600' : 'bg-slate-700'"
          >
            <span
        ...
        <button
          type="button"
          @click="onLogout"
          class="group w-full flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-400 transition-all hover:bg-red-500/15 hover:text-red-400"
        >
          <LogOut
            class="h-4 w-4 shrink-0 text-slate-500 group-hover:text-red-400 transition-colors"
          />
          Log out
        </button>
        ...
        <!-- User info -->
        <div
          class="mt-2 flex items-center gap-3 rounded-lg px-3 py-2.5 border border-white/8 bg-white/5"
        >
          <div
          ...
          <div class="min-w-0 flex-1">
            <p class="truncate text-sm font-medium text-slate-200">{{ auth.user?.username }}</p>
            <p class="truncate text-xs text-slate-500">{{ auth.user?.role }}</p>
          </div>
          <ShieldCheck v-if="auth.isAdmin" class="h-4 w-4 shrink-0 text-blue-400" />
        </div>
      </div>
    </aside>
  </Transition>
